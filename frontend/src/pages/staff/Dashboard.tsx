...
import { Clock, CheckCircle2, AlertCircle, FileText, Play } from 'lucide-react';
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        const pending = complaints.filter((c: any) => c.status === 'ASSIGNED').length;
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-500 dark:text-slate-400">
          Overview of complaints assigned to you.
        </p>
      </div>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
        <Card>
          <CardHeader>
            ...
            <CardDescription>Complaints assigned to you recently</CardDescription>
          </CardHeader>
          <CardContent>
            {recentComplaints.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-10 text-center">
                <AlertCircle className="h-10 w-10 text-slate-300 dark:text-slate-600 mb-4" />
                <h3 className="text-lg font-medium text-slate-900 dark:text-white">No assignments</h3>
                <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
                  You do not have any active complaints assigned to you.
                </p>
              </div>
            ) : (
                ...
                {recentComplaints.map((complaint) => (
                  <div key={complaint.id} className="flex items-center justify-between p-4 rounded-lg border border-slate-200 dark:border-slate-800 transition-all hover:bg-slate-50 dark:hover:bg-slate-800/50">
                    <div className="space-y-1">
                      ...
                      <p className="text-xs text-muted-foreground">{new Date(complaint.createdAt).toLocaleDateString()} • Student: {complaint.student?.fullName}</p>
                    </div>
                    <div className="flex items-center gap-4">
                      ...
                      <Button variant="ghost" size="sm" asChild>
                        <Link to={`/staff/complaints/${complaint.id}`}>View</Link>
                      </Button>
                    </div>
                  </div>
                ))}
